...
public class ModifyBook extends JDialog implements ActionListener{ 
  ...
  public ModifyBook(Frame owner,String title,boolean type,BookTableModel model,int row){ 
    ...
    bookModel=model; 
    rowNum=row; 
    bookNumLabel=new JLabel("学号:"); 
    bookNameLabel=new JLabel("姓名:"); 
    bookWriterLabel=new JLabel("性别:"); 
    bookPublishLabel=new JLabel("年龄:"); 
    bookPriceLabel=new JLabel("班级:"); 
    bookTimeLabel=new JLabel("电话:"); 
    ...
    if(e.getSource()==submitBut){ 
      if(bookNumText.getText().equals("")||bookNameText.getText().equals("")|| 
        ...
        if (n == JOptionPane.YES_OPTION) { 
          String sql="UPDATE Student SET Sname ='"+bookNameText.getText()+"', Ssex= '"+bookWriterText.getText()+"',Sage='"+bookPublishText.getText()+"',Sdept='"+bookPriceText.getText()+"',sushe='"+bookTimeText.getText()+"' WHERE Sno = '"+bookNumText.getText()+"' "; 
          try { 
